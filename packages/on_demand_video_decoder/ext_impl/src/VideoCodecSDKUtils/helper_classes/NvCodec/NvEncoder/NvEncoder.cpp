...
std::vector<std::vector<uint8_t>>  NvEncoder::EncodeFrame(NV_ENC_REGISTERED_PTR ptrRegRes)
{
    ...
    int bufIdx = m_iToSend % m_nEncoderBuffer;

    NV_ENC_PIC_PARAMS picParams;
    picParams.inputTimeStamp = m_iToSend;

    mapInputResource.registeredResource = ptrRegRes;
    NVENC_API_CALL(m_nvenc.nvEncMapInputResource(m_hEncoder, &mapInputResource));
    m_vMappedInputBuffers[bufIdx] =(mapInputResource.mappedResource);
    NVENCSTATUS nvStatus = DoEncode(mapInputResource.mappedResource, m_vBitstreamOutputBuffer[bufIdx], &picParams); 
    ...
    for(int i = 0; m_iGot < iEnd; m_iGot++, i++)
    {
        int bufIdx = (m_iGot) % m_nEncoderBuffer;
        NV_ENC_LOCK_BITSTREAM lockBitstreamData = {NV_ENC_LOCK_BITSTREAM_VER};
        lockBitstreamData.outputBitstream = m_vBitstreamOutputBuffer[bufIdx];
