...
CAIMemoryView coerceToCudaArrayView(py::object cuda_array, NV_ENC_BUFFER_FORMAT bufferFormat, size_t width,
                                    size_t height, int planeIdx) {
    namespace py = pybind11;
    if (!py::hasattr(cuda_array, "__cuda_array_interface__")) {
        throw std::runtime_error(
            "Could not encode CUDA array! Array has no "
    ...
    auto stream = py::hasattr(array_interface, "stream")
                      ? (CUstream)array_interface["stream"].cast<uint64_t>()
                      : (CUstream)2;
    ...
    if (stream == (CUstream)0) {
        throw std::runtime_error(
            "__cuda_array_interface__ protocol specifies that "
    ...
    switch (bufferFormat) {
        case NV_ENC_BUFFER_FORMAT_NV12: {
            if (typestr != "|u1" && typestr != "B")  // TODO: can also be other letters, could use numpy
                                                     // here: np.dtype(typestr)
            {
                throw std::runtime_error("Could not encode CUDA array! Invalid typstr: " + typestr);
            }
            expectedShape = planeIdx == 0 ? std::vector<size_t>{height, width, 1}
                                          : std::vector<size_t>{height / 2, width / 2, 2};
            expectedStrides =
                planeIdx == 0 ? std::vector<size_t>{width, 1, 1} : std::vector<size_t>{width / 2 * 2, 2, 1};
            break;
        }
        ...
        case NV_ENC_BUFFER_FORMAT_ARGB10: {
            throw std::runtime_error(
                "ARGB10 format not supported in current release. "
                "Use YUV444_16BIT or P010");
            if (typestr != "|u1" && typestr != "B")  // TODO: can also be other letters, could use numpy
                                                     // here: np.dtype(typestr)
            {
                throw std::runtime_error("Could not encode CUDA array! Invalid typstr: " + typestr);
        ...
        default:
            throw std::runtime_error(std::string("Could not encode CUDA array! Unsupported color format: ") +
                                     py::str(py::cast(bufferFormat)).cast<std::string>());
    }
