...
"""
``accvlab.on_demand_video_decoder`` - DataLoader Stream Decode Example

This example demonstrates how to use ``accvlab.on_demand_video_decoder`` library with PyTorch DataLoader
for efficient stream-based video decoding in distributed training scenarios. The example shows
...
This example uses CreateSampleReader for sequential/stream access patterns.

Key Features Demonstrated:
- Custom PyTorch Dataset with lazy initialization of GPU decoder (CreateSampleReader)
- Stream-based sequential frame access
...
- Sequential video processing pipelines
"""
...
# .. doc-marker-begin: dataset-stream
class VideoClipDataset(Dataset):
    ...
    This implementation uses CreateSampleReader for stream-based sequential access.
    ...
    Attributes:
        ...
        _nvc_decoder: NVIDIA video decoder instance (CreateSampleReader)
    """
    ...
    def __init__(
        ...
        """
        Initialize the VideoClipDataset for stream access.
    ...
    def _lazy_init(self, index) -> None:
        """
        Lazy initialize the NVIDIA video decoder (CreateSampleReader).
        ...
        try:
            # Important Param: The parameter `num_of_set` is:
            # for a specific instance of nvc.CreateSampleReader that you have created, if you are decoding clipA,
            # after calling DecodeN12ToRGB `num_of_set` times, the input returns to clipA again.
            # If you are continuously decoding the same clip, then num_of_set can be set to 1.
            self._nvc_decoder = nvc.CreateSampleReader(
                num_of_set=self.group_num,  # Cache number_of_set videos status in decoder
                num_of_file=self.num_cameras,  # Maximum number of files to use
                iGpu=self.device_id,  # GPU ID
            )

            self._is_initialized = True
            print(f"Stream video decoder (CreateSampleReader) initialized on GPU {self.device_id}")
        except Exception as e:
            raise RuntimeError(f"Failed to initialize stream video decoder on GPU {self.device_id}: {e}")
    ...
    def __getitem__(self, index: List[Tuple[str, int]]) -> List[List[torch.Tensor]]:
        ...
        for clip_path, frame_idx, _ in index:
            try:
                ...
                # Decode frames from all cameras using CreateSampleReader
...
# .. doc-marker-end: dataset-stream


def main():
    """
    Main function demonstrating stream-based video clip decoding with PyTorch DataLoader.
    ...
    Uses CreateSampleReader for sequential stream access.
    """
    mp.set_start_method('spawn', force=True)

    parser = argparse.ArgumentParser(
        description="Stream video clip decoding example using ``accvlab.on_demand_video_decoder`` (CreateSampleReader) with PyTorch DataLoader"
    )
    parser.add_argument(
    ...
    # Use stream sampler for sequential access
    ...
    # .. doc-marker-end: training-setup-stream
